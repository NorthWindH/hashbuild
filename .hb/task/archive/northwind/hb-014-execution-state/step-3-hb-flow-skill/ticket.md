...
4. The mapping in AC 3 works from **any** valid current state, not only the state's currently-recommended action — e.g. if the recommended action is "execute step 2" but the user says "archive this task instead," the skill still resolves and invokes the archive call-to-action.
...
7. Before carrying out any resolved call-to-action, the skill states the exact invocation it resolved to (skill + target task/step) and asks the user to confirm it:
    1. If the user confirms, the skill invokes that `hb-*` skill directly, in the same session, with the resolved task/step ref as its argument — it delegates the actual work (creating steps, planning, executing, reviewing, archiving) to the existing corresponding `hb-*` skill's own invocation rather than duplicating its steps inline (`hb-flow` is a router/presenter, not a reimplementation).
    2. If the user does not confirm (declines, or asks for something different), the skill asks what to change and re-resolves per AC 3 — it never invokes an invocation that hasn't been confirmed.

---

# Out of scope

- Any change to the behavior of the skills `hb-flow` dispatches to (`hb-task-plan`, `hb-task-step-plan`, `hb-task-archive`, etc.) — they are invoked as-is.
- A confirmation/guard for archiving a task with unexecuted or unreviewed steps — considered and dropped: archiving is trivially reversible via `/hb-task-unarchive`, so no extra protection beyond what `hb-task-archive` already does is required.
- Further `hb-sdk` changes — `hb-flow` only consumes the `hb-sdk state`/`summarize`/`task` surface delivered in the prior two steps.
