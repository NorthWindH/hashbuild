...
def cmd_task_create(args: argparse.Namespace) -> None:
    paths = list[Path]()
    tn = parse_task_name(args.name)

    matching_tasks = find_matching_task_folders(tn)
    ...
    if len(matching_tasks) == 1:
        ...
        if tn != tn_existing:
            die(f"error: task already exists but components mismatched; existing: {tn_existing}, given: {tn}")

    folder = tn.task_id + (f"-{tn.task_extra}" if tn.task_extra else "")

    task_path = path_hb_asserted() / "task" / "active" / tn.author / folder
    ...
    if args.ticket:
        ...
        if not ticket_path.exists():
            die(f"error: ticket file not found: {args.ticket}")
        if ticket_path.suffix != ".md":
            die(f"error: ticket file must end in .md: {args.ticket}")

        ticket_src = ticket_path.read_text()
        ticket_dest_path = path_task_ticket(task_path)
        if not ticket_dest_path.exists():
            progress("importing ticket ...")
            ticket_dest_path.write_text(ticket_src)
            data["ticket_written_at"] = datetime.now(timezone.utc).isoformat()
        else:
            progress("task already has ticket; checking for content difference ...")
            ...
            if ticket_src != ticket_dest:
                ...
                progress("ticket content has changed and --overwrite-ticket supplied; updating ...")
                ticket_dest_path.write_text(ticket_src)
                data["ticket_written_at"] = datetime.now(timezone.utc).isoformat()
            else:
                progress("ticket has not changed, skipping ticket update")
...
def cmd_task_path(args: argparse.Namespace) -> None:
    ...
    if len(matching_tasks) == 0:
        die(f"error: task not found: {args.name}")
    if len(matching_tasks) > 1:
        die(
...
def cmd_task_archive(args: argparse.Namespace) -> None:
    ...
    active_base = path_hb_asserted() / "task" / TASK_FOLDER_ACTIVE
    try:
        task_path.relative_to(active_base)
    except ValueError:
        die(f"error: task is already archived: {task_path.absolute()}")

    archive_author_dir = path_hb_asserted() / "task" / TASK_FOLDER_ARCHIVE / tn.author
    archive_author_dir.mkdir(parents=True, exist_ok=True)

    dest = archive_author_dir / task_path.name
    if dest.exists():
        die(f"error: destination already exists: {dest.absolute()}")
    ...
    progress(f"archived task to {dest.absolute()}")
    paths.append(dest)

    if active_author_dir.exists() and not any(active_author_dir.iterdir()):
        active_author_dir.rmdir()
        progress(f"removed empty author directory: {active_author_dir.absolute()}")
...
def cmd_task_unarchive(args: argparse.Namespace) -> None:
    ...
    archive_base = path_hb_asserted() / "task" / TASK_FOLDER_ARCHIVE
    try:
        task_path.relative_to(archive_base)
    except ValueError:
        die(f"error: task is not archived: {task_path.absolute()}")

    active_author_dir = path_hb_asserted() / "task" / TASK_FOLDER_ACTIVE / tn.author
    ...
    task_path.rename(dest)
    progress(f"unarchived task to {dest.absolute()}")
    paths.append(dest)

    if archive_author_dir.exists() and not any(archive_author_dir.iterdir()):
        archive_author_dir.rmdir()
        progress(f"removed empty author directory: {archive_author_dir.absolute()}")
...
def _find_step_folder(task_path: Path, step_n: int) -> Path:
    ...
    if len(matches) == 0:
        die(f"error: step {step_n} not found in {task_path.absolute()}")
    if len(matches) > 1:
        die(
...
def list_step_folders(task_path: Path) -> list[Path]:
...
def cmd_task_step_add(args: argparse.Namespace) -> None:
    ...
    if not task_json.exists():
        die(f"error: task metadata not found: {task_json.absolute()}")
    ...
    if args.ticket:
        ...
        if not ticket_dest_path.exists():
            ...
            progress("step already has ticket; checking for content difference ...")
            ticket_dest = ticket_dest_path.read_text()
            if ticket_src != ticket_dest:
                if not args.ticket_overwrite:
                    die(
                ...
                progress("ticket content has changed and --ticket-overwrite supplied; updating ...")
                ticket_dest_path.write_text(ticket_src)
            else:
                progress("ticket has not changed, skipping ticket update")
    else:
...
def cmd_task_step_path(args: argparse.Namespace) -> None:
    ...
    matching_tasks = find_matching_task_folders(tn)
    if len(matching_tasks) == 0:
        die(f"error: task not found: {tn.author}/{tn.task_id}")
    if len(matching_tasks) > 1:
        die(
            "error: found multiple existing tasks with same author/task_id:\n"
            + "\n".join(str(p.absolute()) for p, _ in matching_tasks)
