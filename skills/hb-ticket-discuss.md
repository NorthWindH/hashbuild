...
description: >
  ...
  actions (e.g. describe, load, breakdown, clear, push, exit) selectable via
  natural language. Makes no .hb/ writes.
allowed-tools: >
  Write(//tmp/*)
  Write(//private/tmp/*)
  Read(//tmp/*)
...
Run a persistent, multi-turn loop for drafting standalone tickets — no task or step folder is created. Tickets accumulate in in-conversation context across iterations; each iteration presents the current state and a menu of next actions (e.g. describe, load, breakdown, clear, push, exit) selectable via natural language.
